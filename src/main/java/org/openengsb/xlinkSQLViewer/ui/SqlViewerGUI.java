...
public class SqlViewerGUI extends JFrame implements ClipboardOwner{
	...
	public void openXLinkMatch(MatchingResult match){
		...
		SQLCreateModel createStmt = (SQLCreateModel) sqlList.getModel().getElementAt(match.getIndexOfMatch());
		sqlList.ensureIndexIsVisible(match.getIndexOfMatch());
		new SQLCreateDialog(this,createStmt);
	}
	
	/**
	 * Shows a Dialog with the given Message to inform the user that no Match could be found.
	 */
	public void showXLinkMissMatch(String msg){
		JOptionPane.showMessageDialog(this,
				msg,
				"XLink Matching Failed.",
	...
	private class SqlListMouseListener extends MouseAdapter {
	    ...
	    private void maybeShowPopup(MouseEvent e) {
	        if (e.isPopupTrigger()) {
	            ...
	            if (index >= 0) {
	            	selectedStmt = (SQLCreateModel) sqlList.getModel().getElementAt(index);	        
		            logger.log(Level.DEBUG, "Menu on: " + selectedStmt.getTableName());
		        }	            
	        }
	    }	
		...
		public void actionPerformed(ActionEvent e) {
			if(e.getSource() == open){
	            ...
	            if (returnVal == JFileChooser.APPROVE_OPTION) {
	                ...
	                try {
						createStatements = SQLParseUtils.parseCreateStatementsFromFile(file);
