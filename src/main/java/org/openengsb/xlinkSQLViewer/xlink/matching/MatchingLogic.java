...
public class MatchingLogic {
    ...
    /**
     * Searches for all local matches to a given statement and ranks those matches after their similarity to the given
     * statement. The highest ranked match is returned. If no match was found, null is returned. First the name of all
     * tables are matched. If no tables are found or more than one table is found, all statements are compared for their
     * fields.
     */
    ...
    /**
     * Searches the working directory for local matches to a given SQLCreate object from the OpenEngSB. The search is
     * implemented in with an BFS approach.
     */
            ...
            if (file.isFile()) {
                ...
                try {
                    ...
                    int indexInFile = findIndexOfCreateStmt(createStmts,
                            xlinkStmt, true);
                    /*
                     * if (indexInFile == -1) { indexInFile = findIndexOfCreateStmt(createStmts, xlinkStmt, false); }
                     */
                    if (indexInFile != -1) {
    ...
    /**
     * Returns the maximum matching value of the SQL create statements of the given File. This int value indicates, how
     * many many SQLFiels are alike between the two given statements.
     */
